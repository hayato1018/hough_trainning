...
#include <stdio.h>
/*   定数宣言   */
#define MAX_IMAGESIZE 1024    /*   想定する縦・横の最大画素値   */
...
int x_size1,y_size1,x_size2,y_size2;    /* image1,2の横画素数、縦画素数 */
...
/* image1[][], x_size1, y_size1にそれぞれ代入する */
{
    char file_name[MAX_FILENAME];   /* ファイル名用の文字関数 */
    ...
    if(buffer[0] != 'P' || buffer[1] != '5'){
        printf("ファイルのフォーマットがP5をとは異なります.\n");
        exit(1);
    }
    /* x_size1, y_size1の代入(#から始まるコメントは読み飛ばす) */
    x_size1 = 0;
    y_size1 = 0;
...
/* image2[][], x_size2, y_size2 のデータをそれぞれpgm画像、横画素数、縦画素数としてふファイルに保存する */
{
