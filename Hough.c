...
/* image2[y][x] 上の2点(x1, y1),(x2, y2)間の直線上の配列要素の値をインクリメントする*/
{
    ...
    while(t < 1.0){
        x = (int)(t * x2 + (1.0 - t) * x1);
        y = (int)(t * y2 + (1.0 - t) * y1);
        if(x >= 0 && x < x_size2 && y >= 0 && y < y_size2&& image2[y][x] < MAX_BRIGHTNESS){
            image2[y][x] ++;
        }
        t = t + step;
    }
...
int calc_rho(double px, double py, double r_mg, int theta)
...
/* raw = ｘ・cosθ+y・sinθの曲線を、配列image2[rho][theta]に描く */
{
    ...
    for(theta = 0; theta < MAX_THETA; theta++){
        draw_a_straight_line(theta, calc_rho(px, py, rho_c, theta), theta + 1, calc_rho(px, py, rho_c, theta + 1));
    }
}
